...
function renderMedia(items) {
  return `<div class="media-archive">${items.map((item) => (item.type === "video" ? `<video src="${esc(item.src)}" poster="${esc(item.poster)}" muted playsinline controls></video>` : `<img src="${esc(item.src)}" alt="${esc(item.title)}" loading="lazy">`)).join("")}</div>`;
}
function initMenu() {
