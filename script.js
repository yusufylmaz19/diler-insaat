...
async function start() {
  ...
  $("#app").innerHTML = `
    <header class="header"><div class="header-inner wrap"><a class="brand" href="#top"><img src="assets/logo-mark.svg" alt=""><span><b>${esc(data.brand.shortName)}</b><small>İNŞAAT</small></span></a><nav class="nav" id="nav" aria-label="Ana menü">${data.navigation.map((item) => `<a href="${esc(item.href)}">${esc(item.label)}</a>`).join("")}<div class="nav-mobile-contact"><small>PROJENİZİ KONUŞALIM</small><div class="nav-mobile-phones"><a href="${esc(data.brand.phoneHref)}">${esc(data.brand.phone)} <span aria-hidden="true">↗</span></a><a href="${esc(data.brand.secondPhoneHref)}">${esc(data.brand.secondPhone)} <span aria-hidden="true">↗</span></a></div></div></nav><div class="header-call"><span>PROJENİZİ KONUŞALIM</span><div class="header-phones"><a href="${esc(data.brand.phoneHref)}">${esc(data.brand.phone)}</a><a href="${esc(data.brand.secondPhoneHref)}">${esc(data.brand.secondPhone)}</a></div></div><button class="menu-button" aria-controls="nav" aria-expanded="false">Menü</button></div></header>
    ...
    <section class="section references" id="referanslar"><div class="wrap">${sectionHead(data.references)}<div class="reference-grid"><div class="reference-group reveal"><h3>${esc(data.references.contractorsTitle)}</h3><ul>${data.references.contractors.map((item) => `<li>${esc(item)}</li>`).join("")}</ul></div><div class="reference-group reveal"><h3>${esc(data.references.buildingsTitle)}</h3><ul>${data.references.buildings.map((item) => `<li>${esc(item)}</li>`).join("")}</ul></div></div></div></section>
...
function initConstruction(copy) {
  ...
  visual.innerHTML = `<div class="construction-heading"><span>${esc(copy.title)}</span><span class="construction-count" aria-hidden="true">01 / 09</span></div><div class="construction-view"><img class="construction-fallback" src="assets/construction-fallback.svg" alt="${esc(copy.fallback)}"><canvas class="construction-canvas" aria-hidden="true"></canvas></div><div class="construction-caption"><span class="construction-stage" role="status" aria-live="polite">${esc(copy.stages[0])}</span><div class="construction-controls"><button class="construction-control" type="button" hidden>${esc(copy.pause)}</button><button class="construction-scroll-control" type="button" aria-pressed="true">Kaydırmalı animasyon: Açık</button></div></div><div class="construction-progress" aria-hidden="true"><span></span></div><p class="construction-hint">${esc(copy.desktopHint)}</p>`;
  import("./construction.js")
    ...
    .catch((error) => {
      ...
      $(".construction-scroll-control").hidden = true;
    });
}
